...
def clean_ai_response(text):
    ...
    # 保留换行符，但清理多余的空行（超过2个连续换行符的，只保留2个）
    cleaned = re.sub(r'\n{3,}', '\n\n', cleaned)
    
    # 确保每段之间有适当的换行（如果AI没有正确分段，尝试智能分段）
    # 如果文本很长但没有换行，尝试在句号、问号、感叹号后添加换行
    if '\n' not in cleaned and len(cleaned) > 100:
        # 在句号、问号、感叹号后添加换行（但不要在所有标点后都加）
    ...
    def _init_client(self):
        ...
        try:
            ...
            self.client = OpenAI(
                api_key=api_key,
                base_url=base_url,
                http_client=httpx.Client(timeout=300.0)  # 增加到300秒（5分钟），给AI更多分析时间
            )
...
class AIUserPersonalityGenerator:
    ...
    SYSTEM_PROMPT = """你是一位资深情感观察家，同时也是一位社交行为分析师，拥有10年潜水群聊的经验。你的语调要幽默风趣、略带犀利，但最终是温暖的。

你的任务是为群友生成一段多维度、人性化的性格锐评。要求：

【角色设定】
- 你是一位资深情感观察家，拥有10年潜水群聊的经验
- 语调要幽默风趣、略带犀利，但最终是温暖的
- 多用比喻、拟人和口头禅（例如："这老哥是群里的定海神针吧"、"看出来是个气氛组了"）
- 避免使用过于生硬或学术的词汇

【分析维度】
请从以下三个维度进行分析：
1. **核心关注点**：锁定TA的核心兴趣点和社交关系网（最常用词汇、最常@的群友等）
2. **性格推测**：结合用词习惯和发言风格，推测性格特征
3. **群内人设**：评价TA在群内的价值和地位

注意：
- 不要分析发言频率，不要使用"活跃分子"、"万年潜水员"、"消息轰炸机"、"间歇泉"等与发言频率相关的描述
- 不要分析情感倾向，不要使用"正能量输出机"、"吐槽担当"、"中立观察者"等与情感相关的描述
- 重点关注用词特点、兴趣领域、性格特征和群内角色，这些更有差异化

【人性化要求】
- 多用比喻、拟人和口头禅
- 避免使用过于生硬或学术的词汇
- 语气可以是：毒舌吐槽/温情感慨/哲学思考/冷幽默/谐音梗 等
- 要有网感，不要太正经

【举例佐证】
- 在推测性格时，必须引用群友的具体发言作为**『证据』**，避免空泛的评价
- 格式：用引号标注具体发言，例如："从'xxx'这句话可以看出..."

【总结升华】
- 最后，请用一句总结性的、略带哲理的话，评价这位群友在群内的价值或地位
- 例如："TA是群聊中的xxx，xxx的存在让这个群更加xxx"

【输出格式】
请按照以下格式输出（使用换行分段，让文本更易读）：
1. 第一段：用1-2句话概括核心关注点和用词特点（换行）
2. 第二段：分析性格推测，必须引用具体发言作为证据（换行）
3. 第三段：用一句总结性的话评价群内人设

重要：
- 每段之间必须使用换行符（\n）分隔，不要所有内容挤在一段里
- 不要提及发言频率、活跃度、发言数量等与频率相关的信息
- 不要提及情感倾向、正能量、负能量、中立等与情感相关的信息
- 重点关注用词特点、兴趣领域、性格特征，这些更有差异化
- 总长度控制在150-250字之间（分段后可以更长）
"""
    ...
    def generate_personality_comment(self, user_name, representative_words, user_stats=None):
        ...
        top_words = [w['word'] for w in representative_words[:5]]
        
        # 构建用户提示
        stats_text = ""
        if user_stats:
            message_count = user_stats.get('message_count', 0)
            char_count = user_stats.get('char_count', 0)
            avg_chars = user_stats.get('avg_chars_per_msg', 0)
            messages_per_hour = user_stats.get('messages_per_hour', 0)
...
"""
            ...
            stats_text += f"""
【内容特征】
- 最常用词汇 Top 5：{words_text}
"""
            
            # 最常@的群友
            top_at_targets = user_stats.get('top_at_targets', [])
            if top_at_targets:
                at_text = '、'.join([f"{t['name']}({t['count']}次)" for t in top_at_targets])
                stats_text += f"- 最常@的群友：{at_text}\n"
            
            # 情感特征（已移除，不再传递给AI）
            # sentiment = user_stats.get('sentiment', {})
            # if sentiment:
            ...
            #     stats_text += f"""
            # 【情感特征】
            # - 正向发言比例：{positive_ratio:.0%}
            # - 负向发言比例：{negative_ratio:.0%}
            # - 中立发言比例：{neutral_ratio:.0%}
            # """
            
            # 特殊标记
            emoji_count = user_stats.get('emoji_count', 0)
            emoji_usage_rate = user_stats.get('emoji_usage_rate', 0)
            top_emojis = user_stats.get('top_emojis', [])
            if emoji_count > 0:
                stats_text += f"""
【特殊标记】
- 表情符号使用次数：{emoji_count}次
- 表情使用频率：{emoji_usage_rate:.2f}次/条
"""
                if top_emojis:
                    stats_text += f"- 最常用表情：{'、'.join(top_emojis)}\n"
            
            # 发言样本（用于举例佐证）
            message_samples = user_stats.get('message_samples', [])
            if message_samples:
                stats_text += f"""
【发言样本】（用于举例佐证）
"""
                for i, sample in enumerate(message_samples[:3], 1):
                    stats_text += f"{i}. \"{sample[:50]}{'...' if len(sample) > 50 else ''}\"\n"
        
        user_prompt = f"""请为这个群友生成一段多维度、人性化的性格锐评：

群友名称：{user_name}

{stats_text}

请按照系统提示的要求，从核心关注点、性格推测、群内人设这三个维度进行分析。
在推测性格时，必须引用上述发言样本作为证据。
最后用一句总结性的话评价TA在群内的价值或地位。

重要：
- 请使用换行符分段输出，每段之间用换行分隔，让文本更易读
- 不要分析或提及发言频率、活跃度、发言数量等与频率相关的信息
- 不要分析或提及情感倾向、正能量、负能量、中立等与情感相关的信息
- 重点关注用词特点、兴趣领域、性格特征，这些更有差异化
- 直接输出锐评内容，不要加引号或其他格式。"""

        try:
            # httpx.Client的timeout已经设置为300秒（5分钟），确保AI有足够时间完成分析
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": self.SYSTEM_PROMPT},
                ...
                max_tokens=400,  # 增加token数量，支持分段和更详细的点评
                temperature=0.9  # 提高温度，让输出更有创意
            )
    ...
    def generate_batch(self, users_data):
        ...
        print("🤖 正在生成AI群友性格锐评...")
        print(f"   总共需要生成 {len(users_data)} 个用户的锐评，预计需要 {len(users_data) * 30} 秒")
        comments = {}
        for i, user_info in enumerate(users_data, 1):
            user_name = user_info['name']
            print(f"   [{i}/{len(users_data)}] {user_name}...", end=' ', flush=True)
            try:
                ...
                print(f"✗ (失败: {str(e)[:50]})")
                # 失败时使用备用锐评
                comments[user_name] = self._fallback_comment(user_name, user_info['words'])
        
        print(f"✅ 完成！成功生成 {len(comments)} 个锐评")
        return comments
